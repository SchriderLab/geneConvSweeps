...
N=10000
simDir = f"slimSimsForRescsalingTest/PopSize_{N}"
...
for Q in [1, 10]:
    for s in [0.1, 0.01]:
        for h in [0.0, 0.5, 1.0]:
            for gcScalar in [10]:  # this is hard-coded in the SLiM script anyway
                assert gcScalar == 10
                recRateScalar=0
...
for paramComb in paramCombs:
    ...
    for i in range(numBatches):
        ...
        cmd += f"for j in {{0..{repsPerBatch-1}}} ; do slim -d \"dump_file_name='{dumpDir}/{prefix}_batch_{i}_rep_$j.dump'\" -d Q={Q} -d popSize={N} -d unscaledSelCoeff={s} -d domCoeff={h} compare_to_py.slim > {outDir}/{prefix}_batch_{i}_rep_$j.out ; done"
